...
public class deposit extends overView {
    int accountType;
    ...
    public void display(Stage primaryStage, ObservableList<database> customer, int account) {
        ...
        submit.setOnAction(e -> {
            ...
            if ((checkingBalance - AmountTransfered) >= 0 && (savingBalance - AmountTransfered) >= 0 && AmountTransfered >= 0) {
                ...
                alertbox.display("Deposit Successful", "Deposit Successful", accountType, customer);
            } else {
                alertbox = new AlertBox();
                alertbox.display("Deposit unsuccessful", "Not enough balance", accountType, customer);
            }
        });
        if (accountType == 1) {
            howMuch.setText("How much do you want to deposit to checking account?");
            submit.setText("Submit");
            submit.setStyle("-fx-background-color: blue; -fx-text-fill: black");
        } else if (accountType == 2) {
            howMuch.setText("How much do you want to deposit to savings account?");
            submit.setText("Submit");
            submit.setStyle("-fx-background-color: blue; -fx-text-fill: black");
        }
